...
* `validate_ssl` -
  (Optional)
  Boolean specifying whether to include SSL certificate validation as a part of the Uptime check. Only applies to checks where MonitoredResource is set to uptime_url. If use_ssl is false, setting validate_ssl to true has no effect.
